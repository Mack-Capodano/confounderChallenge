...
subtitle: "Visualizing Causal Inference in Surgical Outcomes"
format:
  html: default
...
## Post-Surgical Outcomes by Patient (Observational Data)

The following scatter plot shows post-surgical symptom scores by patient number, with different markers and colors for each doctor:
...
# Plot each doctor's patients
for doctor in ['Doc Dreamy', 'Doc Duck']:
    doctor_data = patients_df[patients_df['doctor_name'] == doctor]
    ax.scatter(doctor_data['patient'], doctor_data['post_surgical_score'],
               c=colors[doctor], marker=markers[doctor], s=80, alpha=0.7,
               label=doctor, edgecolors='black', linewidth=0.5)

# Calculate and plot mean lines
means = patients_df.groupby('doctor_name')['post_surgical_score'].mean()
for doctor in ['Doc Dreamy', 'Doc Duck']:
    mean_val = means[doctor]
    ax.axhline(y=mean_val, color=colors[doctor], linestyle='--', linewidth=2, alpha=0.8)

# Add mean annotations with symbols
ax.annotate(f'○ Mean: {means["Doc Dreamy"]:.2f}',
            xy=(105, means["Doc Dreamy"]), xycoords='data',
            xytext=(5, 0), textcoords='offset points',
            fontsize=12, fontweight='bold', color=colors["Doc Dreamy"],
            bbox=dict(boxstyle="round,pad=0.3", facecolor='white', alpha=0.9))

ax.annotate(f'△ Mean: {means["Doc Duck"]:.2f}',
            xy=(105, means["Doc Duck"]), xycoords='data',
            xytext=(5, 0), textcoords='offset points',
            fontsize=12, fontweight='bold', color=colors["Doc Duck"],
            bbox=dict(boxstyle="round,pad=0.3", facecolor='white', alpha=0.9))
...
ax.set_title('Post-Surgical Symptom Score by Patient and Doctor', fontsize=16, fontweight='bold', pad=20)
...
## Post-Surgical Outcomes by Patient (Randomized Assignment)

To demonstrate how randomization breaks confounding relationships, we examine the same analysis with randomized patient assignment. The following scatter plot uses the same format as the observational data but with randomized assignment:
...
# Create scatter plot for randomized data
fig, ax = plt.subplots(figsize=(10, 6))

# Define colors and markers (same as Figure 1)
colors = {'Doc Dreamy': '#4E79A7', 'Doc Duck': '#E15759'}
markers = {'Doc Dreamy': 'o', 'Doc Duck': '^'}

# Plot each doctor's patients
for doctor in ['Doc Dreamy', 'Doc Duck']:
    doctor_data = patients_randomized_df[patients_randomized_df['doctor_name'] == doctor]
    ax.scatter(doctor_data['patient'], doctor_data['post_surgical_score'],
               c=colors[doctor], marker=markers[doctor], s=80, alpha=0.7,
               label=doctor, edgecolors='black', linewidth=0.5)

# Calculate and plot mean lines
means = patients_randomized_df.groupby('doctor_name')['post_surgical_score'].mean()
for doctor in ['Doc Dreamy', 'Doc Duck']:
    mean_val = means[doctor]
    ax.axhline(y=mean_val, color=colors[doctor], linestyle='--', linewidth=2, alpha=0.8)
...
# Styling (consistent with Figure 1)
ax.set_xlabel('Patient Number', fontsize=14, fontweight='bold')
ax.set_ylabel('Post-Surgical Symptom Score (Lower is Better)', fontsize=14, fontweight='bold')
ax.set_title('Post-Surgical Symptom Score by Patient and Doctor (Randomized Assignment)', fontsize=16, fontweight='bold', pad=20)
ax.grid(True, alpha=0.3)
ax.legend(loc='upper right', fontsize=12)
ax.set_xlim(0, 110)
ax.tick_params(axis='both', which='major', labelsize=12)
...
## Post-Surgical Outcomes by Patient Severity (Stratification)

When randomization is not feasible, stratification provides an alternative approach to control for confounding. The following scatter plot shows post-surgical outcomes as a function of initial patient severity:
...
# Create scatter plot showing outcomes by patient severity
fig, ax = plt.subplots(figsize=(10, 6))

# Define colors and markers (same as previous figures)
colors = {'Doc Dreamy': '#4E79A7', 'Doc Duck': '#E15759'}
markers = {'Doc Dreamy': 'o', 'Doc Duck': '^'}
...
for doctor in ['Doc Dreamy', 'Doc Duck']:
    doctor_data = patients_df[patients_df['doctor_name'] == doctor]
    ax.scatter(doctor_data['severity'], doctor_data['post_surgical_score'],
               c=colors[doctor], marker=markers[doctor], s=80, alpha=0.7,
               label=doctor, edgecolors='black', linewidth=0.5)
...
# Add mean annotations with symbols
ax.annotate(f'○ Mean: {means["Doc Dreamy"]:.2f}',
            xy=(4, means["Doc Dreamy"]), xycoords='data',
            xytext=(0, 0), textcoords='offset points',
            fontsize=12, fontweight='bold', color=colors["Doc Dreamy"],
            bbox=dict(boxstyle="round,pad=0.3", facecolor='white', alpha=0.9))

ax.annotate(f'△ Mean: {means["Doc Duck"]:.2f}',
            xy=(4, means["Doc Duck"]), xycoords='data',
            xytext=(0, 0), textcoords='offset points',
            fontsize=12, fontweight='bold', color=colors["Doc Duck"],
            bbox=dict(boxstyle="round,pad=0.3", facecolor='white', alpha=0.9))

# Add text to explain the shaded region
ax.text(0, 6.5, 'Severity Range: -1 to 1', ha='center', va='bottom',
        fontsize=11, style='italic', color='gray')

# Styling
ax.set_xlabel('Initial Patient Severity', fontsize=14, fontweight='bold')
ax.set_ylabel('Post-Surgical Symptom Score (Lower is Better)', fontsize=14, fontweight='bold')
ax.set_title('Post-Surgical Symptom Score by Patient Severity', fontsize=16, fontweight='bold', pad=20)
ax.grid(True, alpha=0.3)
ax.legend(loc='upper right', fontsize=12)
ax.set_xlim(-3, 5)
ax.tick_params(axis='both', which='major', labelsize=12)
...
Observational data can be misleading when confounding variables influence both treatment assignment and outcomes. In this analysis, patient severity acted as a common cause confounder, making Doc Dreamy appear superior when in fact Doc Duck was the better surgeon.

Randomization provides the gold standard for eliminating confounding, allowing us to estimate true causal effects. When randomization is not feasible, stratification offers a valuable alternative by examining relationships within subgroups of similar individuals.

The key insight is that correlation does not imply causation. Understanding the underlying causal structure—through directed acyclic graphs and careful consideration of confounding—is essential for drawing valid conclusions from data. Randomization when possible, stratification when not: these are the tools that protect us from being misled by numbers.
